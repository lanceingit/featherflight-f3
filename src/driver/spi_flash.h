...
public:  
    ...
    bool eraseCompletely();

    bool pageProgram(uint32_t address, const uint8_t *data, int length);

    bool pageProgramBegin(uint32_t address);
    bool pageProgramContinue(const uint8_t *data, int length);
    void pageProgramFinish();
    ...
    const flashGeometry_t* getGeometry();
...
private:
    ...
    flashGeometry_t _geometry;
    bool _couldBeBusy;
    
    bool performOneByteCommand(uint8_t command);
    bool writeEnable();
    uint8_t readStatus();
